...
public class Slider extends CustomView {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
				...
				if ((event.getX() <= getWidth() && event.getX() >= 0)) {
					...
					if (numberIndicator != null) {
						...
						numberIndicator.indicator.finalY = DpUtils.getRelativeTop(this) - getHeight() / 2;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (value == min) {
			...
			paint.setStrokeWidth(DpUtils.dpToPx(2, getResources()));
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		// Set size of view
		setMinimumHeight(DpUtils.dpToPx(48, getResources()));
		setMinimumWidth(DpUtils.dpToPx(80, getResources()));
		...
		RelativeLayout.LayoutParams params = new LayoutParams(DpUtils.dpToPx(20, getResources()), DpUtils.dpToPx(20,
				getResources()));
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (animate) {
				...
				y -= DpUtils.dpToPx(6, getResources());
				size += DpUtils.dpToPx(2, getResources());
			}
			canvas.drawCircle(ViewHelper.getX(ball) + DpUtils.getRelativeLeft((View) ball.getParent()) + ball.getWidth()
					/ 2, y, size, paint);
			if (animate && size >= finalSize)
				animate = false;
			if (animate == false) {
				ViewHelper.setX(numberIndicator.numberIndicator,
						(ViewHelper.getX(ball) + DpUtils.getRelativeLeft((View) ball.getParent()) + ball.getWidth() / 2)
								- size);
